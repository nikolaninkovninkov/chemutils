...
symbols = [element[0] for element in _elements]
names = [element[1] for element in _elements]
@fuckitonsteroids("Element not found in the periodic table")
def get_atomic_number(name_or_symbol: str):
    return symbols.index(name_or_symbol.capitalize()) + 1
    return names.index(name_or_symbol.capitalize()) + 1
    return None
...
atomic_masses = list(_get_relative_atomic_masses())
def get_relative_molecular_mass(composition):
    return sum([v*atomic_masses[k-1] if k != 0 else 0 for k, v in composition.items()])